...
class MyProduct(models.Model):
    ...
    user_id = models.ForeignKey(User, related_name="user_product", on_delete=models.CASCADE, db_column="id", verbose_name="사용자 id")

    # id = 자동생성
    my_product_id = models.AutoField(primary_key=True)
    #키 = 유저id + 문서 종류 + 문서번호로 생성, ex) 00000000 + 문서코드(6~8) + max로 추가 채번, 필요하면 사용
    #my_product_key = models.CharField(max_length=50, unique=True, verbose_name="내 제품키", primary_key=True)
    ...
    storage_method = models.TextField(max_length=1000, verbose_name="보관방법", null=True, blank=True)
    ...
    importer_address = models.CharField(max_length=500, verbose_name="수입원 및 소재지", null=True, blank=True)
    
    cautions = models.TextField(max_length=1000, verbose_name="주의 사항", null=True, blank=True)
    additional_info = models.TextField(max_length=1000, verbose_name="기타 표시 사항", null=True, blank=True)
    
    create_datetime = models.DateTimeField(auto_now_add=True)
    update_datetime = models.DateTimeField(auto_now=True)
    label_create_YN = models.CharField(max_length=1, verbose_name="표시 사항 작성 여부", null=True, blank=True )
    ...
    delete_YN = models.CharField(max_length=1, verbose_name="내 제품 삭제 여부", null=True, blank=True )
...
class MyLabel(models.Model):
    ...
    create_datetime = models.DateTimeField(auto_now_add=True)
    update_datetime = models.DateTimeField(auto_now=True)
    #label_create_YN = models.CharField(max_length=1, verbose_name="표시 사항 작성 여부" )
