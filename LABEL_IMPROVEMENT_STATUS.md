...
#### P4-6. 남은 구멍 — V2 에 입력칸이 없는 항목 3개 ★

자동 세팅으로 켜지는 항목 중 셋은 **V2 기본정보 탭에 입력칸이 없다.**
켜지면 "필수 미입력" 이 뜨는데 그 화면에서 채울 수가 없다.

| 항목 | V2 입력칸 | 실제로 채우는 곳 |
|---|---|---|
| `nutrition_text` 영양성분 | 없음 | 영양성분 탭 (별도) |
| `weight_calorie` 내용량(열량) | 없음 | V1 작성 화면에만 있음 |
| `rawmtrl_nm_display` 원재료명(표시) | **없음** | V1 작성 화면에만 있음 |

특히 세 번째가 문제다. V2 기본정보 탭에 "원재료명 표시명" 이라는 라벨이 붙은
칸이 있는데 **실제 `name` 은 `rawmtrl_nm`(참고용 파생 필드)** 이다.
`rawmtrl_nm` 은 relation 에서 다시 생성되는 값이라 사용자가 적은 게 덮어써질 수
있고, 정작 인쇄되고 검증 대상인 `rawmtrl_nm_display` 는 V2 어디에도 없다.

**이건 이번 범위에서 고치지 않았다.** 라벨 문구와 필드가 어긋난 것을 어느 쪽에
맞출지(칸을 `rawmtrl_nm_display` 로 바꿀 것인가, 라벨 문구를 "원재료명(참고)" 로
바꿀 것인가)는 화면 동작을 보고 정할 일이다. 다음 작업 후보 1순위.
...
현재 71개. 저장소의 기존 회귀 방지 관례는 Django system check
