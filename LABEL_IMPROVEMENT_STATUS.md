...
**별건으로 남긴 이름 불일치**: V2 기본정보 탭의 "원재료명 표시명" 이라는 라벨이
붙은 칸은 실제로는 `rawmtrl_nm`(참고용 파생 필드)이다. 그 값은 relation 에서 다시
생성되므로 사용자가 적은 게 덮어써질 수 있다. 폴백 덕에 인쇄에는 지장이 없지만
이름과 동작이 어긋나 있다. 어느 쪽에 맞출지는 화면 동작을 보고 정할 일.
...
현재 103개. 저장소의 기존 회귀 방지 관례는 Django system check
