...
기본정보 탭에 카드 하나(`#section-display-items`)를 넣어 18개 체크박스를
한눈에 보여준다. 식품유형이 정한 값을 배지로 함께 표시한다(**필수** / **해당없음**),
켜져 있는데 값이 빈 항목은 **미입력** 배지가 붙는다.

저장은 `product_update_fields` 가 받는다. 화면이 **체크된 것만이 아니라 18개
전부**를 보낸다 — 켠 것만 보내면 서버가 "끈 것" 과 "화면에 없던 것" 을 구분할
수 없다. 식품유형이 **바뀐 경우에만** 규칙을 덧씌운다(안 그러면 저장할 때마다
사용자가 끈 항목이 되살아나 끌 수가 없다).
...
현재 90개. 저장소의 기존 회귀 방지 관례는 Django system check
