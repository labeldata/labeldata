...
class OCRResult(models.Model):
    ...
    processed_image = models.ImageField(
        upload_to='ocr_processed/%Y/%m/%d/',
        null=True,
        blank=True,
        verbose_name="전처리된 이미지"
    )
    
    # OCR 결과
    raw_text = models.TextField(
        verbose_name="원본 OCR 텍스트",
        blank=True
    )
    structured_data = models.JSONField(
        verbose_name="구조화된 데이터",
        default=dict,
        blank=True
    )
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['user_id', 'status'], name='idx_ocr_user_status'),
            models.Index(fields=['created_at'], name='idx_ocr_created'),
        ]

    def __str__(self):
        return f"OCR-{self.id} ({self.user_id.username})"
...
class OCRFieldMapping(models.Model):
    ...
    class Meta:
        db_table = "ocr_field_mapping"
        indexes = [
            models.Index(fields=['ocr_result', 'field_name'], name='idx_ocr_field'),
        ]
        unique_together = ['ocr_result', 'field_name']

    def __str__(self):
        return f"{self.field_name}: {self.extracted_text[:50]}..."
