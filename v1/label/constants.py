...
# ─────────────────────────────────────────────────────────────────────────
CALORIE_FACTORS = {
    'carbohydrates': 4.0,    # 아래 '탄수화물에서 빼는 것' 을 뺀 나머지
    'dietary_fiber': 2.0,
    'sugar_alcohols': 2.4,
    'proteins': 4.0,
    'fats': 9.0,
    ...
    # 식약처 「영양성분 등록 요령」의 열량 계산식에 이름이 따로 나오는 당류다.
    # **당알코올로 묶으면 안 된다.** 에리스리톨은 화학적으로 당알코올이지만
    # 요령은 2.4 가 아니라 **0** 으로 센다 — 10 g 쓴 제품에서 24 kcal 이
    # 허공에서 생긴다. 타가토스·알룰로오스도 각자 계수가 있다.
    'tagatose': 1.5,
    'allulose': 0.0,
    'erythritol': 0.0,
}

# 탄수화물 안에 들어 있으면서 계수가 다른 것들. 탄수화물에서 이만큼을 빼고
# 각자의 계수로 센다(요령의 계산식이 괄호 안에서 하는 일이 이것이다).
CALORIE_CARB_PARTS = ('dietary_fiber', 'sugar_alcohols',
                      'tagatose', 'allulose', 'erythritol')

# 열량 계산값과 표시값이 얼마나 벌어져도 되는가 — 요령이 정한 **±20 %**.
#
# 이 수를 여기 두는 이유는 **두 곳이 서로 다른 잣대를 쓰고 있었기** 때문이다.
# 라벨 검사는 30 %(느슨)였고 적재 검산은 15 %(빡빡)였다. 어느 쪽도 규정의
# 수가 아니었고, 그래서 적재 검산은 규정이 받아 주는 행을 어긋났다고 적었다 —
# '배추김치_가을재배' 재계산 30.1 kcal / 표기 37.0 은 18.6 % 차라 요령으로는
# 통과다(김치의 유기산은 이 DB 에 컬럼 자체가 없어 우리 계산이 늘 낮게 나온다).
NUTRITION_CALORIE_TOLERANCE = 0.20

# **보이지 않는 유기산 몫.**
#
# 열량을 탄단지로 세는 모든 자리가 같은 구멍을 갖는다 — 유기산(3 kcal/g)과
# 알코올(7 kcal/g)은 식약처 영양성분DB 에 **컬럼 자체가 없다**
# (mfds_nutrition.NOT_IN_SOURCE). 그 몫은 우리 눈에 안 보인 채 표기 열량에만
# 들어 있으므로, 계산값과 견줄 때 그만큼은 얹어 주어야 한다.
#
# 일반 식품에서 유기산은 6~7 g/100 g 을 넘기 어렵다 — 약 20 kcal 이다.
# 김치(표기 37.0 · 계산 30.7)와 양조식초(20.0 · 0.4)가 이 여유로 제자리를 찾는다.
# 주류는 이것으로 못 덮는다(17 도 소주는 에탄올만 117 kcal). 이름으로 뺀다 —
# nutrition_anomaly.ALCOHOL_WORDS.
NUTRITION_ORGANIC_ACID_SLACK_KCAL = 20.0
