...
- nutrition_basis: 표의 값이 **무엇 당**인지. 표에서 두 곳을 함께 보고 적는다.

        ┌────────────────────────────────────┐
        │ 영양정보    총 내용량 100 g         │  ← 기준량
        │             96 kcal                 │
        ├────────────────────────────────────┤
        │ 총 내용량당   1일 영양성분 기준치… │  ← **무엇 당인지**
        │ 나트륨 660 mg                33 %  │
        └────────────────────────────────────┘

    값 칸의 열 머리("총 내용량당" / "100 g당" / "1회 제공량당")를 그대로
    옮기고, 표 머리에 기준량이 적혀 있으면 그 양도 함께 적는다.
    예: "총 내용량 100 g 당", "100 g당", "1회 제공량 30 g당"
    **열 머리가 "100 g당" 이면 그것이 기준이다** — 표 머리의 총 내용량이
    500 g 이어도 값들은 100 g 당이다. 둘을 바꿔 적으면 모든 수치가 다섯 배로
    틀어진다.
    **열 머리에 "…당" 이 아예 없는 표도 많다.** 성분을 두 줄로 나눠 적는
    가로형 표가 특히 그렇다 — 그때는 표 머리의 총 내용량이 기준이므로
    "총 내용량 133 g" 처럼 그 표기를 그대로 적는다. 비워 두지 마시오.
...
# 칸(알레르기)을 망쳤다. 편차가 80을 넘는 회차도 있었다. 그래서 한동안 아예
# 읽지 않았다 - 화면에 빠른 입력 버튼 스물여덟 개가 있으니 손으로 채우라고.
#
# 운영에서 그 판단이 뒤집혔다. 라벨에 인쇄된 문장은 버튼 목록에 없는 것이
# 많았고, 사진을 올린 사람은 그 칸이 왜 비어 있는지부터 물었다. 지금은
# **기본으로 읽는다**(settings.OCR_READ_FREETEXT). 읽은 값은 확인 창을 반드시
# 거치므로, 지어낸 문장은 사용자가 체크를 풀어 버릴 수 있다.
#
# `.env` 에 OCR_READ_FREETEXT=False 를 넣으면 예전처럼 비운다. 측정 화면은
# 이 값과 무관하게 껐다 켜서 견준다.
...
# 원문을 함께 보낼 때의 지시문.
#
# 처음에는 문서함 PDF 와 같은 말을 썼다 - "글자는 원문을 그대로 옮기고 사진은
# 배치 판단에만 쓰라". 정답지 넷을 재 보니 그 말이 **너무 넓었다.**
#
#     rawmtrl_nm    +14.5  +19.8  +79.5  +25.0   네 장 모두 크게 올랐다
#     allergens     -87.7  -33.9                 두 장에서 무너졌다
#     nutrition_basis -31.8 -53.8
#     prdlst_nm     -11.1  -37.9
#
# 무너진 셋은 **원문을 그대로 옮기면 안 되는 칸**이다. 알레르기는 "함유" 를
# 떼고 물질 이름만 적어야 하고, 제품명은 작업지시서 품명을 걸러야 하고,
# 영양성분 기준은 표의 머리글이라 표 모양을 봐야 안다. 그런데 "한 글자도
# 바꾸지 말라" 고 했으니 모델이 그 지시를 따른 것이다.
#
# PDF 는 문서 한 장이 통째로 본문이라 그 말이 통했다. 사진은 다르다 - 라벨
# 밖의 작업지시서 표까지 원문에 섞여 들어온다.
#
# 그래서 **어느 칸에 쓸지를 정해 준다.** 긴 문구는 원문을 그대로, 나머지는
# 사진을 먼저.
_HYBRID_INSTRUCTION = (
    "\n\n아래는 이 사진에서 **기계가 그대로 읽어 낸 글자**입니다. "
    "**원문은 참고이지 정답이 아닙니다.** 어디에 쓸지가 정해져 있습니다.\n"
    "\n"
    "[원문을 그대로 옮길 항목] 원재료명, 주의사항, 기타표시사항\n"
    "  길고 그대로 옮겨야 하는 문구입니다. 원문에 있는 글자를 **한 글자도 바꾸지 "
    "말고** 옮기세요. 다듬거나 요약하지 마세요. 한 줄이 원문 여기저기에 흩어져 "
    "있으면 사진의 배치를 보고 이어 붙이세요.\n"
    "\n"
    "[나머지 항목] 사진을 먼저 믿으세요\n"
    "  원문에는 **라벨 밖의 글자도 섞여 있습니다.** 작업지시서 표, 인쇄 기호, "
    "재단선 표기가 그대로 들어갑니다. 어느 글자가 라벨 안에 있는지는 사진만 "
    "압니다.\n"
    "  그리고 원문을 그대로 옮기면 **틀리는** 항목이 있습니다. 이 항목들은 "
    "위의 시스템 지시를 그대로 따르세요.\n"
    "    - 알레르기: \"함유\" 를 뺀 물질 이름만. 혼입가능 물질은 넣지 마세요\n"
    "    - 제품명: 작업지시서의 품명이 아니라 라벨 안의 제품명\n"
    "    - 영양성분 기준(nutrition_basis): 표의 머리글. 표 모양은 사진으로 봐야 압니다\n"
    "    - 분리배출 표시: 도형입니다. 원문에 없으니 사진에서 읽으세요\n"
    "\n"
...
def drop_freetext(data, read_freetext=None):
    ...
    if read_freetext is None:
        read_freetext = getattr(settings, 'OCR_READ_FREETEXT', True)
    if read_freetext:
        return data
...
#
# **목록에서 빠진 항목은 안 읽힌다.** 지시문이 "각 장에 적힌 것만 읽으세요" 와
# 함께 나가기 때문에, 여기 이름이 없는 칸은 모델이 아예 찾지 않는다. 실제로
# 일괄표시면을 골라 읽었더니 원산지·유통전문판매원·주의사항·기타표시사항이
# 통째로 비어서 왔다 - 사진에는 다 인쇄돼 있었다. 그래서 **그 면에 실제로
# 인쇄되는 것을 빠짐없이** 적는다.
REGION_ROLES = {
    'main':      ('주표시면',      '제품명, 내용량, 내용량(열량), 특정성분 함량'),
    'info':      ('일괄표시면',    '식품유형, 품목보고번호, 원재료명, 알레르기 표시, '
                                   '원산지, 제조원·유통전문판매원·소분원·수입원, '
                                   '소비기한, 보관방법, 포장재질, 주의사항, '
                                   '기타표시사항(소비자상담실·교환장소·인증표시)'),
    'rawmtrl':   ('원재료명 영역', '원재료명 및 함량, 알레르기 표시, 원산지'),
    ...
    try:
        ...
        content.append({
            ...
            "text": (
                ...
                f"'여기서 찾을 항목' 은 그 면에 **흔히** 있는 것을 짚어 준 것이지 "
                f"거기까지만 읽으라는 뜻이 아닙니다. 목록에 없는 항목이라도 그 장에 "
                f"적혀 있으면 읽으세요.\n"
