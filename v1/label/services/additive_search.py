...
# Y/null 플래그 컬럼의 선택지 (용도 플래그, 표4·5·6 대상 여부)
_FLAG_CHOICES = ['Y']
_PURPOSE_CHOICES = _FLAG_CHOICES


def _purpose_conditions():
    """
    용도 플래그(착색료·감미료 등)를 조건으로 펼친다.
    모델의 PURPOSE_FIELDS 를 그대로 쓰므로 용도가 추가돼도 여기만 따라온다.
    """
    from v1.label.models import FoodAdditive
    return [
        {'key': field, 'label': f'용도: {label}', 'field': field,
         'lookup': 'iexact', 'type': 'choice', 'choices': _PURPOSE_CHOICES}
        for field, label in FoodAdditive.PURPOSE_FIELDS
    ]
...
def _base_conditions():
    ...
    return [
        ...
        {'key': 'alias_4', 'label': '표시기준: 표4 (명칭+용도)', 'field': 'alias_4',
         'lookup': 'iexact', 'type': 'choice', 'choices': _FLAG_CHOICES},
        {'key': 'alias_5', 'label': '표시기준: 표5 (명칭·간략명)', 'field': 'alias_5',
         'lookup': 'iexact', 'type': 'choice', 'choices': _FLAG_CHOICES},
        {'key': 'alias_6', 'label': '표시기준: 표6 (명칭·간략명·용도)', 'field': 'alias_6',
         'lookup': 'iexact', 'type': 'choice', 'choices': _FLAG_CHOICES},
    ]
...
def conditions_catalog():
    ...
    if _conditions_cache is None:
        _conditions_cache = _base_conditions() + _purpose_conditions()
