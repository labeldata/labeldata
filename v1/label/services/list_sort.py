...
# 어느 열을 볼 것인가.
#
# 목록의 열이 회사마다 다르게 쓰인다. 어떤 회사는 GMO 를 매번 보고 어떤
# 회사는 한 번도 안 본다. 안 보는 열이 자리를 차지하면 정작 봐야 할 원료명이
# 좁아진다.
#
# **필수 열은 끌 수 없다.** 이름과 식별자까지 끄면 무엇을 보고 있는지 알 수
# 없게 된다. 나머지는 사용자가 정하고, 그 선택은 **계정에 남는다** — 사내
# 기준은 그 사람이 일하는 방식이라 자리를 옮긴다고 달라지지 않는다
# (배합비 붙여넣기 순서와 같은 자리, UserProfile.list_prefs).
# ─────────────────────────────────────────────────────────────────────────
ESSENTIAL_FIELDS = frozenset({'prdlst_nm', 'my_label_name', 'ingredient_name'})


def hidden_for(user, screen: str) -> set:
    """이 사용자가 이 화면에서 꺼 둔 열."""
    try:
        prefs = (getattr(user, 'profile', None).list_prefs or {})
        saved = (prefs.get('hidden_columns') or {}).get(screen)
    except Exception:
        return set()
    return set(saved) if isinstance(saved, list) else set()


def save_hidden(user, screen: str, fields) -> set:
    """꺼 둔 열을 계정에 남긴다. 필수 열은 받아도 무시한다."""
    profile = getattr(user, 'profile', None)
    if profile is None:
        return set()
    clean = sorted({str(f) for f in (fields or ())
                    if str(f) and str(f) not in ESSENTIAL_FIELDS})
    prefs = dict(profile.list_prefs or {})
    hidden = dict(prefs.get('hidden_columns') or {})
    hidden[screen] = clean
    prefs['hidden_columns'] = hidden
    profile.list_prefs = prefs
    profile.save(update_fields=['list_prefs'])
    return set(clean)


def visible(specs, hidden) -> list:
    """꺼 둔 열을 뺀 목록. 필수 열은 꺼져 있어도 남는다."""
    hidden = set(hidden or ())
    return [c for c in specs
            if c.get('field') in ESSENTIAL_FIELDS or c.get('field') not in hidden]


def toggles(specs, hidden) -> list:
    """
    켜고 끄는 자리에 그릴 목록.

        [{'field', 'label', 'on', 'essential'}, …]
    """
    hidden = set(hidden or ())
    out = []
    for spec in specs:
