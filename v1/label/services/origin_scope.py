...
제외되는 것과, 그 제외가 순위에 미치는 영향은 조문이 한 문장으로 정한다.
...
빼면 준초콜릿이 3 순위가 된다. 조문대로다.)

그리고 **당류가공품이 명시돼 있다.** 당류를 주원료로 한 가공품은 이름이
'당류가공품' 이거나 식품유형이 그것이다.
...
# 당류가공품 — 조문이 **이름을 박아** 제외에 넣은 것이다("당류를 주원료로
# 하여 가공한 당류가공품을 포함"). 위의 _SUGARS 와 달리 통째로 같지 않아도
# 본다: 식품유형이 '당류가공품' 이고 이름이 '당류가공품(분말)' 처럼 꼬리를
# 다는 일이 흔하다. 이 낱말을 품은 다른 물건은 없다.
_SUGAR_PRODUCT = '당류가공품'
