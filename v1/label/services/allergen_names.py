...
# 표의 **열 이름**으로 쓰는 말.
#
# 배합비를 엑셀로 관리하는 사람들은 알레르기를 이렇게 적는 일이 흔하다.
#
#     계란  우유  밀  대두
#      O    X    X   O
#
# 그 열 이름을 알아보려면 이름 목록이 필요한데, 판정용 키워드 전체
# (ALLERGEN_KEYWORDS)를 쓰면 "간장"·"두부" 같은 **원료 이름까지** 알레르기
# 열로 오해한다. 열 머리에 실제로 적는 말만 추린다.
#
# 값은 표시 명칭이다. "계란" 열에 O 가 있으면 "알류" 로 넣는다 — 규정이
# 요구하는 이름이고, 뒤에서 이 값을 키로 쓰는 곳이 그 이름을 찾는다.
HEADER_NAMES = {
    '알류': ['알류', '계란', '달걀', '난류'],
    '우유': ['우유', '유제품'],
    '메밀': ['메밀'],
