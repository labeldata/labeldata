...
def normalize_report_no(value):
    ...
    '202509629419' 옆에 '2020_DNSP_04044' 같은 값이 섞여 온다. 글자가 섞인
    값은 조인 키로 쓰지 않는다 — 억지로 숫자만 뽑아 맞추면 엉뚱한 원료에
    붙는다('2020_DNSP_04044' → '202004044' 가 남의 번호일 수 있다).

    **다만 하이픈과 공백은 뗀다.** 같은 번호가 두 꼴로 온다 —
    '20220460436160' 과 '19980448010-697'. 예전에는 하이픈이 붙은 것을 통째로
    버렸는데, 그 번호들은 멀쩡한 품목이고 버리면 조인이 그만큼 줄어든다.
    표시사항 쪽 조회(`item_lookup.find_exact`)는 이미 두 꼴을 함께 본다.
    """
    text = (value or '').strip()
    if not text:
        return ''
