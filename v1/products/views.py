...
_WORKFLOW_STEPS = (
    ...
    # 이 탭에서 사람들이 가장 많이 하는 일은 **검증**이다. 그런데 이름이
    # "표시사항" 이라 표를 보는 자리로만 보였고, 두 검증(표시문구·디자인시안)이
    # 서로 다른 레이어에 흩어져 있어 같은 종류로 보이지도 않았다.
    ('tab-label', '검증',
     '표시사항을 표로 그리고, 1차(표시문구)·2차(디자인시안) 검증을 여기서 합니다.'),
)
