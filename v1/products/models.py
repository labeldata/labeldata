...
class DocumentType(models.Model):
    ...
    # 한 제품에 여러 건이 쌓이는 구분인가
    #
    # **판 쌓기는 '같은 문서의 새 판' 일 때만 맞다.** 품목제조보고서를 다시
    # 올리면 앞의 것을 대신하는 새 판이지만, 원료 표시사항은 **장마다 다른
    # 원료**다. 크림치즈 사진 다음에 설탕 사진을 올리면 그것은 크림치즈의
    # 새 판이 아니다.
    #
    # 그런데 업로드는 슬롯이 없는 구분을 전부 '같은 구분의 옛 문서에 판으로
    # 붙이기' 로 다뤘다. 그래서 원료 사진을 셋 올리면 목록에는 마지막 것
    # 한 줄만 남고 앞의 둘은 판 접힘 속으로 들어갔다 — 지운 적도 없는데
    # 사라진 것으로 보인다.
    multiple_yn = models.BooleanField(
        default=False,
        verbose_name="여러 건 허용",
        help_text="켜면 올릴 때마다 새 문서가 됩니다. 끄면 앞 문서의 새 판이 "
                  "됩니다. 원료 표시사항처럼 장마다 다른 것을 담는 구분에 켭니다."
    )
