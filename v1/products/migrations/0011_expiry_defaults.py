...
예외는 성적서다. 자가품질검사성적서와 영양성분분석서는 **발행일로부터 얼마**
라는 관행이 있고(6개월), 문서 자체에 만료일이 안 적혀 있는 일이 흔하다.
그때는 기본값이 실제로 쓸모가 있다.
"""
from django.db import migrations

# 6개월. 앱 곳곳이 영양성분분석서도 '성적서' 라고 부른다(영양성분 탭의
# 「성적서 판독」·「성적서에서 가져오기」가 이 구분을 가리킨다).
SIX_MONTHS = ('TEST_QUALITY', 'ANALYSIS_NUTRITION')
DAYS = 180
