...
class RegulatoryNews(models.Model):
    ...
    # ── 원본 데이터 ──────────────────────────────────────────────
    # API 출처 코드: I2620 | I2640 | I0490 | I0470 | I0480 | import
    api_source   = models.CharField(max_length=20, blank=True, default='',
                                    verbose_name='API 출처 코드', db_index=True,
                                    help_text='수집 API 서비스 ID (I2620/I2640/I0490/I0470/I0480/import)')
    source       = models.CharField(max_length=20, choices=SOURCE_CHOICES,
                                    verbose_name='출처', db_index=True)
    external_id  = models.CharField(max_length=100, db_index=True,
                                    verbose_name='외부 고유번호',
                                    help_text='국내: cntntsSn, 수입: API 고유키')
    ...
    # ── AI 분석 결과 ─────────────────────────────────────────────
    # 예: ["프로바이오틱스", "프락토올리고당", "유산균"]
    ai_keywords  = models.JSONField(default=list, verbose_name='AI 추출 키워드',
                                    help_text='원재료명 + 검출물질 배열 (매칭 및 UI 표시용)')
    # 원재료 목록: [{name, origin, manufacturer, food_type}, ...]
    ai_issues    = models.JSONField(default=list, blank=True, verbose_name='AI 구조화 이슈',
                                    help_text='텍스트에 명시된 원재료 구조화 배열')
    # 검출된 물질명 (농약명, 균명, 중금속명, 첨가물명 등)
    ai_substances = models.JSONField(default=list, blank=True, verbose_name='검출 물질',
                                     help_text='텍스트에 명시된 검출 물질명 배열')
    # 위반 유형 코드: pesticide|microbe|heavy_metal|additive|foreign_body|
    #                deficiency|labeling|admin|import_ban|other
    violation_type = models.CharField(max_length=30, blank=True, default='',
                                      verbose_name='위반유형 코드', db_index=True)
    ai_summary   = models.TextField(blank=True, verbose_name='AI 요약')
    ai_parsed    = models.BooleanField(default=False, verbose_name='AI 분석 완료',
                                       db_index=True)

    risk_level    = models.CharField(max_length=4, choices=RISK_CHOICES,
                                     default=RISK_MED, verbose_name='위험도')

    # ── 수집 메타 ────────────────────────────────────────────────
    # 실제 발생/처분일 (imp_insp: impotDtm 부적합일, 국내: 처분결정일/회수개시일)
    event_date     = models.DateField(null=True, blank=True, verbose_name='발생/처분일',
                                      db_index=True,
                                      help_text='imp_insp=수입검사부적합일, 국내=처분·회수일')
...
class NewsProductMatch(models.Model):
    ...
    product = models.ForeignKey(
        'label.MyLabel',
        on_delete=models.CASCADE,
        related_name='regulatory_matches',
        verbose_name='연관 제품'
    )
    # 어떤 BOM 원료가 매칭되었는지 (옵션)
    matched_bom = models.ForeignKey(
        'bom.ProductBOM',
        on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name='regulatory_matches',
        verbose_name='매칭된 BOM 항목'
    )
    ...
    risk_level  = models.CharField(max_length=4, choices=RISK_CHOICES,
                                    default=RISK_LOW, verbose_name='매칭 위해도')
    risk_score  = models.IntegerField(default=0, verbose_name='위해도 점수(0~100)')
    risk_reasons = models.JSONField(default=list, blank=True,
                                    verbose_name='위해도 산정 근거',
                                    help_text='["원료명 일치(마늘종)", "원산지 일치(중국)"]')

    # ── 알림 / 확인 상태 ─────────────────────────────────────────
    is_read  = models.BooleanField(default=False, verbose_name='확인 여부', db_index=True)
    read_at  = models.DateTimeField(null=True, blank=True, verbose_name='확인 일시')

    # ── 오탐지 신고 ───────────────────────────────────────────────
    is_false_positive    = models.BooleanField(default=False, verbose_name='오탐지 신고',
                                               db_index=True)
    false_positive_at    = models.DateTimeField(null=True, blank=True,
                                                verbose_name='오탐지 신고 일시')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['product', 'is_read']),
...
class NewsIngredientMatch(models.Model):
    ...
    news       = models.ForeignKey(
        RegulatoryNews, on_delete=models.CASCADE,
        related_name='ingredient_matches', verbose_name='부적합 정보'
    )
    user       = models.ForeignKey(
        User, on_delete=models.CASCADE,
        related_name='ingredient_news_matches', verbose_name='사용자'
    )
    ingredient = models.ForeignKey(
        'label.MyIngredient', on_delete=models.CASCADE,
        related_name='news_matches', verbose_name='원료'
    )

    matched_keyword = models.CharField(max_length=200, verbose_name='매칭 키워드')
    match_score     = models.FloatField(default=0.0, verbose_name='매칭 점수')
    risk_level      = models.CharField(max_length=4, choices=RISK_CHOICES,
                                       default=RISK_LOW, verbose_name='등급')
    risk_score      = models.IntegerField(default=0, verbose_name='위해도 점수')

    is_read      = models.BooleanField(default=False, db_index=True, verbose_name='확인 여부')
    is_dismissed = models.BooleanField(default=False, db_index=True, verbose_name='해당 없음')
...
class RegulatoryMatchAction(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE,
                             related_name='regulatory_actions', verbose_name='사용자')

    # 둘 중 하나만 값을 가짐 (제품 매칭 또는 원료 매칭)
    product_match    = models.ForeignKey(
        NewsProductMatch, on_delete=models.CASCADE,
        null=True, blank=True, related_name='actions', verbose_name='제품 매칭'
    )
    ingredient_match = models.ForeignKey(
        NewsIngredientMatch, on_delete=models.CASCADE,
        null=True, blank=True, related_name='actions', verbose_name='원료 매칭'
    )
