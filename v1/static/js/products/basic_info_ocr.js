 ...
 */
  ...
  function applySelected() {
    /* **이 제품에 손을 댔다고 알린다.**
     *
     * '사진으로 시작' 으로 들어온 화면은 떠날 때 "손대지 않은 빈 제품" 을
     * 지운다(product_detail.html 의 pagehide 비콘). 그 판정은 **서버가
     * 저장된 값만 보고** 하는데, 여기서 채우는 것은 대부분 폼일 뿐이라
     * 저장은 사람이 따로 눌러야 한다 — 서버 눈에는 여전히 빈 제품이다.
     *
     * 그래서 새로고침 한 번에 작업하던 제품이 통째로 지워졌다. discard 는
     * 하드 삭제라 되돌릴 것도 없다.
     *
     * 아래에서 폼을 채우면 checkFormChanges 가 hasUnsavedChanges 를 켜 주고
     * 비콘은 그것도 본다. 이 표시는 그 그물에 안 걸리는 칸(칩·배지처럼
     * 추적 대상이 아닌 것)까지 덮기 위한 것이다. */
